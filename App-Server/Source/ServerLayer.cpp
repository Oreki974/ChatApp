...
void ServerLayer::OnAttach()
{
	...
	m_MessageHistoryFilePath = "MessageHistory.yaml";//历史聊天记录
...
void ServerLayer::OnUIRender()//渲染右侧客户端信息
{
#ifndef WL_HEADLESS
	{
		...
		for (const auto& [id, name] : m_ConnectedClients)//按id，name依次遍历
		{
...
{
	if (!std::filesystem::exists(filepath))//检测目录
		return false;
